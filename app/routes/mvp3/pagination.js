const applications = require('../../data/applications.json'); // Update the path accordingly

function getPaginatedResults(req, res) {
    const pageSize = 25; // Set the number of items per page
    const totalCount = applications.length; // Total items
    const pageNumber = parseInt(req.query.page) || 1; // Get the page number from the query params
    const pageCount = Math.ceil(totalCount / pageSize); // Calculate total pages

    // Calculate the start index for the current page
    const startIndex = (pageNumber - 1) * pageSize;

    // Get the items for the current page, limited to 25
    const paginatedData = applications.slice(startIndex, startIndex + pageSize);

    // Render your view/template and pass the pagination variables
    res.render('/search_1', {
        applications: paginatedData,
        pageCount,
        pageNumber,
        firstResultNumber: startIndex + 1,
        lastResultNumber: Math.min(startIndex + pageSize, totalCount),
        totalCount
    });
}
...
function Pagination(data, pageNumber = 1, pageSize = 25) {
    this.data = data
