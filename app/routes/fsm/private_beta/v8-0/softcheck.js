module.exports = function(router) {
  ...
  router.post('/FSM/Private_beta/v8-0/family/start-now', (req, res) => {
      req.session.data.user = {};
      res.redirect('/FSM/Private_beta/v8-0/family/parent-soft-check/check');
  });
  ...
  router.post('/FSM/Private_beta/v7-2-soft-check-ni-answer', function (req, res) {
    ...
    if (niNumber === "yes") {
      if (enteredNiNumber) {
        res.redirect('/FSM/Private_beta/v8-0/family/parent-soft-check/checking-loader');
      } else {
        res.redirect('/FSM/Private_beta/v8-0/family/parent-soft-check/error-ni');
      }
    } else {
      res.redirect('/FSM/Private_beta/v8-0/family/parent-soft-check/nass-number');
    }
  });
  ...
  router.post('/nass-number-answer', function (req, res) {
    ...
    if (nassNumber === "yes") {
      res.redirect('/FSM/Private_beta/v8-0/checker-parent/childs-age');
    } else {
      res.redirect('/FSM/Private_beta/v8-0/checker-parent/more-info-required-asylum');
    }
  });
