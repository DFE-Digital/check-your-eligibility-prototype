...
////v7-4///
  ///have-evidence///
  router.post('/v7-4-evidence', function (req, res) {
    var evidence = req.session.data['evidence'];
    if (evidence === "v7digital") {
      res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/evidence/upload-guidance-digital');
    } else if (evidence === "v7paper") {
      res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/evidence/upload-guidance');
    } else {
      res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/add-child/child-details-complete-0-none.html');
    }
  });

  /// gov routes///
  // router.post('/v7-1-gov', function (req, res) {
  //   res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/evidence/have-evidence.html');
  // });

  router.post('/v7-4-gov-2', function (req, res) {
    res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/add-child/child-details-0.html');
  });

  router.get('/FSM/Private_beta/v7-4/family/parent-soft-check/outcomes/outcome-not-entitled-appeal', (req, res) => {
    req.session.data.startingPage = 'appeal-process';
    res.render('/FSM/Private_beta/v7-4/parent-soft-check/outcomes/outcome-not-entitled-appeal');
  });

  router.get('/FSM/Private_beta/v7-4/family/parent-soft-check/outcomes/eligible', (req, res) => {
    req.session.data.startingPage = 'eligible';
    res.render('v7-4/family/parent-soft-check/outcomes/eligible');
  });
  ...
  router.post('/v7-1-success-0', function (req, res) {
    res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/add-child/children-added-0.html');
  });

      ///success///
      router.post('/v7-1-success-none', function (req, res) {
        res.redirect('/FSM/Private_beta/v7-4/family/account/appeal/add-child/children-added-0-none.html');
      });
};
