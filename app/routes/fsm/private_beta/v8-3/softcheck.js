...
router.get('/FSM/Private_beta/v8-3/LA/la-manage/la-soft-check/run-check', function (req, res) {
  ...
  res.render('FSM/Private_beta/v8-3/LA/la-manage/la-soft-check/run-check')
})

router.post('/FSM/Private_beta/v8-3/LA/la-manage/la-soft-check/run-check', function (req, res) {
...
// router.post('/FSM/Private_beta/v8-3/LA/la-manage/apply/check-answers', function (req, res) {
//   console.log('CHECK ANSWERS ROUTE HIT (SOFTCHECK)')
//   res.redirect('/FSM/Private_beta/v8-3/LA/la-manage/apply/la-check-answers.html')
// })
...
var result = eligibilityLookup[nino] || eligibilityLookup['PN123456D']
    ...
    res.redirect('/FSM/Private_beta/v8-3/LA/la-manage/la-soft-check/checking-la-loader.html')
  });


router.post(
  '/FSM/Private_beta/v8-3/school/school-manage/school-soft-check/run-check',
  function (req, res) {
    ...
    res.redirect(
      '/FSM/Private_beta/v8-3/school/school-manage/school-soft-check/school-checking-loader.html'
    )
  }
)
  ...
  router.post('/FSM/Private_beta/v8-3/family/start-now', (req, res) => {
      req.session.data.user = {};
      res.redirect('/FSM/Private_beta/v8-3/family/parent-soft-check/check');
  });

  // Route for soft check NI answer
  router.post('/FSM/Private_beta/v8-3-soft-check-ni-answer', function (req, res) {
    var niNumber = req.session.data['ni-number'];
    var enteredNiNumber = req.session.data['ni-number-entered'];

    if (niNumber === "yes") {
      if (enteredNiNumber) {
        res.redirect('/FSM/Private_beta/v8-3/family/parent-soft-check/checking-loader');
      } else {
        res.redirect('/FSM/Private_beta/v8-3/family/parent-soft-check/error-ni');
      }
    } else {
      res.redirect('/FSM/Private_beta/v8-3/family/parent-soft-check/nass-number');
    }
  });
  ...
  router.post('/nass-number-answer', function (req, res) {
    ...
    if (nassNumber === "yes") {
      res.redirect('/FSM/Private_beta/v8-3/checker-parent/childs-age');
    } else {
      res.redirect('/FSM/Private_beta/v8-3/checker-parent/more-info-required-asylum');
    }
  });
