...
router.get('/FSM/Private_beta/v8-4/LA/la-manage/la-soft-check/run-check', function (req, res) {
  ...
  res.render('FSM/Private_beta/v8-4/LA/la-manage/la-soft-check/run-check')
})
...
var result = eligibilityLookup[nino] || eligibilityLookup['PN123456D']
    ...
    res.redirect('/FSM/Private_beta/v8-4/LA/la-manage/la-soft-check/checking-la-loader.html')
  });


router.post(
  '/FSM/Private_beta/v8-4/school/school-manage/school-soft-check/run-check',
  function (req, res) {
    ...
    res.redirect(
      '/FSM/Private_beta/v8-4/school/school-manage/school-soft-check/school-checking-loader.html'
    )
  }
)
  ...
  router.post('/FSM/Private_beta/v8-4/family/start-now', (req, res) => {
      req.session.data.user = {};
      res.redirect('/FSM/Private_beta/v8-4/family/parent-soft-check/check');
  });

  // Route for soft check NI answer
  router.post('/FSM/Private_beta/v8-4-soft-check-ni-answer', function (req, res) {
    var niNumber = req.session.data['ni-number'];
    var enteredNiNumber = req.session.data['ni-number-entered'];

    if (niNumber === "yes") {
      if (enteredNiNumber) {
        res.redirect('/FSM/Private_beta/v8-4/family/parent-soft-check/checking-loader');
      } else {
        res.redirect('/FSM/Private_beta/v8-4/family/parent-soft-check/error-ni');
      }
    } else {
      res.redirect('/FSM/Private_beta/v8-4/family/parent-soft-check/nass-number');
    }
  });
  ...
  router.post('/nass-number-answer', function (req, res) {
    ...
    if (nassNumber === "yes") {
      res.redirect('/FSM/Private_beta/v8-4/checker-parent/childs-age');
    } else {
      res.redirect('/FSM/Private_beta/v8-4/checker-parent/more-info-required-asylum');
    }
  });
